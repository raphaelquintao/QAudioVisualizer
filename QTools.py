...
class QAnim(object):
    import os, gi, time, cairo, cv2, threading, gc
    gi.require_version('Gtk', '3.0')
    from gi.repository import GLib, Gtk, Gdk, GdkPixbuf

    def __init__(self, path="images/bg.gif", width=800, height=400, max_fps=24, show_fps=False):
        self.path = path
        self.width = width
        self.height = height
    ...
    def load(self, path):
        thread = self.threading.Thread(target=self._load, args=(path, ), daemon=True)
        thread.start()
        # thread.join()
        self.gc.collect()
    ...
    def _load(self, path):
        ...
        # --- OPENCV ---
        # vidcap = self.cv2.VideoCapture('images/bg.png')
        # vidcap = self.cv2.VideoCapture('videos/monsters.mp4')
        vidcap = self.cv2.VideoCapture(path)

        w, h = (vidcap.get(3), vidcap.get(4))
        fps = vidcap.get(self.cv2.CAP_PROP_FPS)
        num_frames = vidcap.get(self.cv2.CAP_PROP_FRAME_COUNT)
        nw, nh = self._get_new_size(w, h)
        ...
        print("CURR_FPS:", min(fps, self.max_fps) )

        count = 0
        success, frame = vidcap.read()
        while success:
            frame = self.cv2.cvtColor(frame, self.cv2.COLOR_BGR2RGB)
            # frame = self.cv2.resize(frame, (int(nh),int(nw)))

            # result, image = self.cv2.imencode('.jpg', frame, [int(self.cv2.IMWRITE_JPEG_QUALITY), 50])
            # frame = self.cv2.imdecode(image, 1)
            # del image

            pixbuf = self.GdkPixbuf.Pixbuf.new_from_data(frame.tobytes(), self.GdkPixbuf.Colorspace.RGB, False, 8, w, h, w * 3)
            del frame

            # self.frames.append(pixbuf)
            frames.append(pixbuf.scale_simple(nw, nh, self.GdkPixbuf.InterpType.TILES))
            del pixbuf

            # if(count > fps * 8): break
    ...
    def _get_new_size(self, w, h):
        delta1 = self.height / h
        delta2 = self.width / w
        delta = max(delta1, delta2)

        return (w * delta, h * delta)
    ...
    def tick(self):
        ...
        fps_delta = 1000 / min(self.fps, self.max_fps)
        # fps_delta = self.delay

        # self.darea.queue_draw()
        self.last = (self.last + 1) % len(self.frames);
        self.image.set_from_pixbuf(self.frames[self.last])

        self.GLib.timeout_add(int(fps_delta), self.tick)

    def start(self):
        def update():
            self.tick()
        thread = self.threading.Thread(target=update, daemon=True)
        thread.start()
